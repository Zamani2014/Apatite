<%@ Page Language="C#" AutoEventWireup="true" CodeFile="LatestRegistersUsers.aspx.cs" Inherits="Administration_LatestRegistersUsers" MasterPageFile="~/Site.master" %>

<asp:Content ID="Content1" ContentPlaceHolderID="mainContent" Runat="Server">
<fieldset>
<legend>آخرین کاربران ثبت سپرده ملکی</legend>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        DeleteCommand="DELETE FROM [RegisterTblUsers] WHERE [ID] = @original_ID AND [CompName] = @original_CompName AND [CompIP] = @original_CompIP AND [CompDate] = @original_CompDate AND [CompTime] = @original_CompTime AND [CompBrowser] = @original_CompBrowser AND (([Referer] = @original_Referer) OR ([Referer] IS NULL AND @original_Referer IS NULL))" 
        InsertCommand="INSERT INTO [RegisterTblUsers] ([ID], [CompName], [CompIP], [CompDate], [CompTime], [CompBrowser], [Referer]) VALUES (@ID, @CompName, @CompIP, @CompDate, @CompTime, @CompBrowser, @Referer)" 
        OldValuesParameterFormatString="original_{0}" 
        SelectCommand="SELECT * FROM [RegisterTblUsers]" 
        UpdateCommand="UPDATE [RegisterTblUsers] SET [CompName] = @CompName, [CompIP] = @CompIP, [CompDate] = @CompDate, [CompTime] = @CompTime, [CompBrowser] = @CompBrowser, [Referer] = @Referer WHERE [ID] = @original_ID AND [CompName] = @original_CompName AND [CompIP] = @original_CompIP AND [CompDate] = @original_CompDate AND [CompTime] = @original_CompTime AND [CompBrowser] = @original_CompBrowser AND (([Referer] = @original_Referer) OR ([Referer] IS NULL AND @original_Referer IS NULL))">
        <DeleteParameters>
